...
#Функция, для ожидания пока наша backup job завершится, чтобы дождаться пока backup выполнится перед удалением mysql deployment, svc, pv, pvc.
...
#Функция, для обработки Jinja шаблонов и преобразования YAML в JSON:
...
#Cоздание бэкапов и восстановление из них (логику удаления успешно законченных jobs c определенным именем)
...
def mysql_on_create(body, spec, **kwargs):
    ...
    #генерация json из шаблона для restore-job
